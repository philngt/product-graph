...
/** Presentation coordinator over the existing working graph. No second model or API. */
export function createStudioPages({ state, focus, inspect, lens, model, review }) {
  ...
  function renderBuild() {
    ...
    return `<div class="page-heading"><div><p class="eyebrow">TARGET INSPECTION</p><h2 id="page-heading" tabindex="-1">From model to application</h2><p>Inspect the inputs. Compiler support is a separate milestone.</p></div><span class="tag warning">SwiftUI generation not implemented</span></div>
      <div class="build-pipeline"><section class="paper-card"><span class="card-index">01</span><h3>Product graph</h3><strong class="large-number">${state.graph.nodes.length}</strong><p>Objects in the full working model.</p><button class="text-button" data-build-model>Open model →</button></section><span aria-hidden="true">→</span><section class="paper-card"><span class="card-index">02</span><h3>Template definitions</h3><strong class="large-number">${templates.length}</strong><p>Local metadata, not executable generator support.</p><button class="text-button" data-build-library>Inspect templates →</button></section><span aria-hidden="true">→</span><section class="paper-card unavailable"><span class="card-index">03</span><h3>SwiftUI output</h3><p>No compiler or generated project is connected to this workspace yet.</p><button class="button" disabled>Generate application</button></section></div>
      <section class="paper-card build-boundary"><h3>What is available now?</h3><p>Editing, relationship exploration, saved validation, context and proposal review use the current workspace. This screen does not compile code, run Xcode, or claim production readiness.</p><p>Nothing is generated by opening or inspecting a template. Your canonical graph remains unchanged.</p></section>`;
  }
  function render() {
    ...
    if (page === 'build') $('page-content').innerHTML = renderBuild();
