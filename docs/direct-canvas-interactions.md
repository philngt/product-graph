...
- `ui/app.js`: uses the existing working graph, single Undo snapshot, dirty/context invalidation, navigation and Save pipeline. Sidebar Add/Connect actions delegate to the same controller; old text prompts are removed.
...
The existing Studio browser regression uses the new explicit form instead of mocked prompts, and emits the click following its synthetic pointer-up. It remains a separate full-UI fixture suite and needs its ordinary prerequisites. No historical pass count is claimed as a current full-suite result.
