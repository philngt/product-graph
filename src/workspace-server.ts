  ...
  return async (request: http.IncomingMessage, response: http.ServerResponse) => {
    try {
      ...
      if (scoped) {
        ...
        const allowed = /^\/(workspace|graph|context|agent-context|task-context|implementation(?:\/context)?|authoring(?:\/(?:board|proposal))?|compare|commands|proposals(?:\/[^/]+(?:\/(?:preview|apply|reject))?)?|documents(?:\/read)?)$/;
